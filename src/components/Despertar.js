import React from 'react';

import cave from '../assets/cave.png'
import lotus from '../assets/lotus.png'


export default function Despertar() {
    return(
        <>

            <div className="bg-meditation">
                <div className="container">
                    <div className="row">
                        <div className="col-12">
                            <p className="text-right">"Ainda que tenhamos encontrado os monstros dentro de nós, vamos continuar<br/> 
                            tentando matá-los no mundo exterior. Porém, toda a escuridão do Mundo vem<br/> 
                            do coração e é lá que devemos trabalhar."<br/>
                            <i>Marianne Williamsom</i>
                            </p>
                        </div>
                    </div>
                </div>
            </div> 

            <div className="text-amor">
                <div className="container">
                    <div className="row">
                        <div className="col-12 col-md-10 offset-md-1">
                            <h2>Você quer realmente se conectar com seu Eu superior e viver uma vida preenchida<br/> 
                            com amor, abundância e alegria?
                            </h2>

                            <p>	A maioria de nós, sem saber que merecemos e podemos mais, nos acomodamos com uma vida que pode parecer incompleta, em que somente vivemos um dia após o outro, esperando que de alguma maneira as coisas melhorem para que então possamos encontrar a felicidade.
                            </p>    

                            <p>E se eu te falar que você pode ser feliz agora? Que neste exato momento você pode se conectar com quem você realmente é e nesta conexão não há nenhum sentimento de descontentamento, desmerecimento ou dor?
                            </p>
                            
                        </div>
                    </div>
                </div>
            </div> 

            <div className="spiritual-coach">
                <div className="container">
                    <div className="row">
                        <div className="col-12">
                            <p className="col-12 col-md-10 offset-md-1">Um Espiritual Coach te ajuda a mergulhar dentro de si para curar seu corpo, sua mente e seu coração, permitindo acessar seu verdadeiro poder pessoal. Uma vez que tenha as ferramentas e o entendimento de que você também é um milagre e pode conquistar tudo que desejar, você irá então se libertar das limitações da mente e do auto e coletivo condicionamento que vem te aprisionando durante toda a vida.</p>

                            <h3>Se está cansado de se deparar com as mesmas situações e emoções negativas e quer mudar sua vida 
                            para melhor; ou se experienciou um Despertar Espiritual espontâneo, estou aqui para ajudá-lo 
                            a caminhar com consciência, confiança e gratidão.
                            </h3>

                            <p className="col-12 col-md-10 offset-md-1">Quando eu passei pelo meu próprio despertar anos atrás, me deparei com um caminho libertador mas também muito assustador. Eu me libertei por meio do sofrimento e aprendi que existem outras formas de subir a montanha. A partir daí,  ajudar as pessoas  em suas jornadas rumo à Liberdade se tornou minha missão.</p>

                            <p className="col-12 col-md-10 offset-md-1">Eu também me senti perdida, desmotivada e assustada e procurei ajuda com outros Coaches Espirituais. Alguns me ensinaram muito e me ajudaram a me tornar confiante e me reerguer para receber a graça que se derramava sobre mim, mas isto foi somente após ter passado por outros profissionais que estavam exclusivamente interessados em ganhos financeiros, me deixando afogar em meu desespero com sessões intermináveis que não me ajudavam da maneira que eu necessitava. Tendo vivenciado isto, eu hoje trabalho para ajudar genuinamente meus clientes a adquirirem conhecimento e confiança para que possam percorrer seus caminhos sozinhos.</p>
                        </div>
                    </div>
                </div>
            </div> 

            <div className="bg-cave">
                <div className="container">
                    <div className="row">
                        <div className="col-12">
                            <img src={cave} className="img-left" alt="" />

                            <h3>O caminho para me encontrar</h3>

                            <p>	Desde criança tenho tido experiências incomuns, como esse episódio que gostaria de dividir com vocês: aconteceu quando eu tinha por volta de cinco anos de idade. Estava com muita fome e queria
                            logo tomar minha sopa, mas ela estava quente demais, então pedi a Deus que ajudasse a resfriá-la,
                            no que não somente ouvi como senti um sopro suave no meu rosto que esfriou a sopa  	          			instantaneamente. Assustada eu corri e contei a minha mãe, mas ela não acreditou em mim.
                            (como ela poderia?).</p>

                            <p>Outra experiência foi quando eu tinha 20 anos, e um homem desconhecido se aproximou e disse que Deus tinha uma mensagem para mim: "Eu estou sempre com você, em todos os momentos. Quando você se escondia na beira do rio a noite para chorar, eu também estava lá ao seu lado. Como Jo, tudo foi tirado de você, como Jo, você também será honrada.” 
                            </p>

                            <p>Até aquele dia eu nunca tinha comentado com ninguém minhas idas ao rio próximo à casa da<br/>
                            minha família para chorar. Sou muito grata por esta mensagem, que veio para me dar forças para enfrentar muitas outras<br/>  situações difíceis que ainda estavam por vir.</p>

                        </div>

                        <div className="col-12 col-md-4">
                            <h4>Eu sempre senti que "Deus",<br/>
                            ou como você queira chamar,<br/>
                            estava olhando por mim.</h4>
                        </div>

                        <div className="col-12 col-md-8">
                            <p>Isto me deu uma coragem inabalável para correr atrás dos meus objetivos, mas até aquele momento eu via Deus como algo muito longe de mim. Um supremo poder que eventualmente me ouviria se eu implorasse o suficiente ou chorasse o suficiente ou  me encontrasse em total desespero.</p>
                        </div>
                    </div>
                </div>
            </div>

            <div className="sintomas">
                <div className="container">
                    <div className="row">
                        <div className="col-12 col-md-10 offset-md-1">
                            <p>Somente após meu Despertar Espiritual que percebi que Ele habita dentro de mim. Que o mesmo poder que cria  Universos, também faz meu coração bater e digere o alimento que eu consumo. Aí então entendi o que Osho quer dizer com a frase: "Ore como um amante e não como um mendigo". Graças ao Despertar Espiritual agora estou conectada à fonte divina de criação, não coloco barreiras ao fluxo da vida, que é sempre tão generoso e amável, e aprendi também que ele sempre esteve disponível, basta nos abrir verdadeiramente à Graça que espera pacientemente seus filhos ao retornarem para casa.
                            </p>
                        </div>

                        <div className="col-12 col-md-3 offset-md-1">
                            <h4>Sintomas físicos do<br/> 
                            Despertar Espiritual:</h4>
                        </div>

                        <div className="col-12 col-md-7">
                            <p>	Apesar de levar à Liberdade verdadeira e ao amor incondicional, o despertar normalmente vem acompanhado da fase de expansão e incorporamento deste  poder superior, na qual haverão inúmeras transformações. É quando começamos a nos desprender de padrões e crenças antigas, nossas emoções ficam à flor da pele e além disso estamos enfrentando sintomas físicos que podem ser difíceis de controlar, especialmente se precisamos trabalhar e parecer "normais" interagindo com outras pessoas.</p>

                            <p><strong>Abaixo alguns dos sintomas físicos mais comuns que identifiquei e aprendi a tratar:</strong></p>
                        </div>

                        

                        <div className="col-12 col-md-4 offset-md-3 list-ansiedade">
                            <ul>
                                <li>
                                    - Ansiedade e medo do desconhecido
                                </li>

                                <li>
                                    - Pressão e dores na cabeça, peito e costas
                                </li>

                                <li>
                                    - Palpitações cardíacas
                                </li>

                                <li>
                                    - Insônia
                                </li>
                            </ul>
                        </div>

                        <div className="col-12 col-md-4 list-fadiga">
                            <ul>
                                <li>
                                    - Fadiga
                                </li>

                                <li>
                                    - Correntes de energia/eletricidade por todo o corpo
                                </li>

                                <li>
                                    - Sentidos ampliados
                                </li>

                                <li>
                                    - Ondas de calor
                                </li>
                            </ul>
                        </div>
                        
                    </div>
                </div>
            </div>

            <div className="bg-lotus">
                <div className="container">
                    <div className="row">

                        <div className="col-9 offset-md-2">
                            <p>Os sintomas acima são somente físicos, normalmente vêm acompanhados de enormes mudanças mentais e emocionais, que abrirão espaço para o indivíduo se tornar seu Eu superior e ser então guiado por sua própria fonte de intuição e poder.
                            </p>
                        </div>

                        <div className="col-12 col-md-4">
                            <img src={lotus} alt="" />
                        </div>

                        <div className="col-12 col-md-7 ferramentas">
                            <p>	Com as ferramentas e comportamentos corretos que possibilitem a expansão da Consciência, a pessoa perceberá que estamos todos conectados ao Espírito e quando abrirmos a porta para receber as bênçãos que o Universo está disposto a proporcionar, começaremos então a manifestar reais milagres.</p>

                            <p className="col-8 offset-md-4 lotus-box">
                            Entre em contato para agendar sua sessão online<br/>
                             preenchendo o quadro abaixo ou enviando e-mail para<br/>
                            andressa@andressa-awakeningminds.com
                            </p>
                        </div>
                    </div>
                </div>
            </div>
            
               
            <div className="bg-black">
                <div className="container">
                    <div className="row">
                        <div className="col-12">
                            <p>Se você está passando por Emergência Espiritual ou Síndrome da Kundalini e está com dificuldades financeiras, envie também um e-mail explicando sua situação e<br/> farei o possível para te ajudar: andressa@andressa-awakeningminds.com
                            </p>
                        </div>
                    </div>
                </div>
            </div> 



    
            








        </>

    )
}